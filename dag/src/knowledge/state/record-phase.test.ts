  ...
  it("covers orchestrator phases used by the DAG loop", () => {
    for (const event of LOOP_ORCHESTRATOR_PHASE_EVENTS) {
      expect(LOOP_EVENT_CHECKPOINT_PHASE[event]).toMatch(
        /^(PLAN_CREATED|NODE_STARTED|CONTEXT_BUILT|RED_STARTED|GREEN_STARTED|VERIFICATION_STARTED|VERIFICATION_COMPLETED|COMMIT_CREATED|NODE_COMPLETED|NODE_FAILED)$/,
      );
    }
  });
});

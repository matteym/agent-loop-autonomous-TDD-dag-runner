...
Up to 5 fix sends. Then `dag/logs/failures.log`. The orchestrator **PAUSE**s instead of dying: `n` → `git reset --hard HEAD` (tracked only, no `git clean -fd`), history `status=failed`, exit non-zero; `o` → do not revert, skip archive, NEXT node. Node is not archived on either answer. Missing keys / missing DAG file: PAUSE, then exit 1 after `n` or a second `o`. Non-TTY stdin is treated as `o`. Agent protocol (this file) is otherwise unchanged.
